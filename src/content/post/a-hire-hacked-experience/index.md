...
上面说到我已经删掉了 github 上的 ssh key，准备重新生成一个。之前生成的 ssh key都是不带密码的，也就是说私钥是明文保存在硬盘上的，这次我打算新生成一个 ssh key，设置一个比较复杂的密码，这样即使泄露，不是高价值人群，攻击者也不会花精力去破解我的密码，只要密码不太常用，爆破的话几乎无法破解。

有了密码之后，每次使用 私钥都需要输入密码，非常麻烦，这时候可以利用 `ssh-agent`，只要第一次输入密码，就会一直留在内存中。
